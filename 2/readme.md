...
| **yaw** | **水平消失點**的 x 偏移（結構化場景才有） | ⚠️ 結構化場景 ✅，否則 N/A |

> **yaw 的進階做法（消失點法）**：純看一張圖原本拿不到 yaw，但在**結構化（Manhattan）場景**——走廊、建築、街道——「平行的水平線會在影像上收斂到一個**水平消失點**」，其相對主點的 x 偏移即對應相機 yaw（`orient.image_pose`）。線條近乎平行（消失點在無限遠）時 yaw 不可觀測 → 標 `N/A`。
> 消失點姿態是**經典 CV 技術**（非單目運動法），本專案以自有的 `vanishing_point()` 實作。實測：結構化照片（建築/街道）得 yaw ≈ −22°～−24°，開闊/雜亂場景則退回 `N/A`。

得到 `(yaw, pitch, roll)` 後用 `ypr_to_R` 建旋轉矩陣，餵給與影片**同一個** XYZ 指示器繪製。實作見 `src/orient.py`。
